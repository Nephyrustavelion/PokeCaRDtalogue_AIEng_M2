...
const STATIC_RARITIES = [
  ...
  'Common', 'Crown', 'Double rare', 'Four Diamond', 'Full Art Trainer',
  'Futuristic Rare', 'Holo Rare', 'Holo Rare V', 'Holo Rare VMAX',
  'Holo Rare VSTAR', 'Hyper rare', 'Illustration rare', 'LEGEND',
  'Mega Hyper Rare', 'None', 'One Diamond', 'One Shiny', 'One Star',
  'Pikachu Rare', 'Promo', 'Radiant Rare', 'Rare', 'Rare Holo',
  'Rare Holo LV.X', 'Rare PRIME', 'Secret Rare', 'Shiny Ultra Rare',
  'Shiny rare', 'Shiny rare V', 'Shiny rare VMAX', 'Special illustration rare',
  'Three Diamond', 'Three Star', 'Two Diamond', 'Two Shiny', 'Two Star',
  'Ultra Rare', 'Uncommon',
]
...
async function fetchJson(url, retries = 2) {
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`TCGdex request failed with ${response.status}`)
...
export async function fetchCardSetNames() {
  ...
  if (!config) throw new Error('TCGdex environment configuration is missing or invalid')
  const sets = await fetchJson(`${config.baseUrl}/en/sets`)
  const names = Array.isArray(sets) ? sets.map((set) => set.name).filter(Boolean) : []
 ...
 */
export async function fetchCardsPage({ search, types, rarities, setNames, page, limit }) {
  const config = getTcgDexApiConfig()
  if (!config) throw new Error('TCGdex environment configuration is missing or invalid')
  ...
  for (let i = 0; i < results.length; i += DETAIL_BATCH_SIZE) {
    ...
    const settled = await Promise.allSettled(
      batch.map((card) => fetchJson(`${config.baseUrl}${config.endpoint}/${card.id}`)),
    )
